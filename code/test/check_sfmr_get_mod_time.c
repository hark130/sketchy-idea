/*
 *  Check unit test suit for skip_file_metadata_read.h's get_mod_time() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_get_mod_time.bin
code/dist/check_sfmr_get_mod_time.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_get_mod_time.bin
...
#include "skip_file_metadata_read.h"  // get_mod_time()
