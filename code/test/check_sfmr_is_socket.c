/*
 *  Check unit test suit for skid_file_metadata_read.h's is_socket() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_is_socket.bin
code/dist/check_sfmr_is_socket.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_is_socket.bin
...
#include "skid_file_metadata_read.h"  // is_socket()
