/*
 *  Check unit test suit for skid_file_metadata_read.h's get_change_time() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_get_change_time.bin
code/dist/check_sfmr_get_change_time.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_get_change_time.bin
...
#include "skid_file_metadata_read.h"  // get_change_time()
