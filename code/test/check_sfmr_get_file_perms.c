/*
 *  Check unit test suit for skid_file_metadata_read.h's get_file_perms() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_get_file_perms.bin
code/dist/check_sfmr_get_file_perms.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_get_file_perms.bin
