...
/*
 *	Replicate the behavior of read_dir_contents() using a combination of devops_code's remove_a_file()
 ...
 *	before calling read_dir_contents() and attempt to manually remove it all if read_dir_contents() fails.
 */
	               ...
	               unsigned int tree_width, unsigned int tree_depth)
{
	...
	// RUN IT
	// Call the function
	// actual_ret = read_dir_contents(dir_input);
	// Compare actual results to expected results
	ck_assert_msg(exp_return == actual_ret, "read_dir_contents(%s) returned [%d] '%s' "
