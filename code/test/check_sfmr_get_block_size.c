/*
 *  Check unit test suit for skid_file_metadata_read.h's get_block_size() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_get_block_size.bin
code/dist/check_sfmr_get_block_size.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_get_block_size.bin
