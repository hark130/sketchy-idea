/*
 *  Check unit test suit for skip_file_metadata_read.h's is_sym_link() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_is_sym_link.bin
code/dist/check_sfmr_is_sym_link.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_is_sym_link.bin
