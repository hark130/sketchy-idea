/*
 *  Check unit test suit for SKID_file_metadata_read.h's is_named_pipe() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_is_named_pipe.bin
code/dist/check_sfmr_is_named_pipe.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_is_named_pipe.bin
