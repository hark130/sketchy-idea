/*
 *  Check unit test suit for skid_file_metadata_read.h's is_block_device() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_is_block_device.bin
code/dist/check_sfmr_is_block_device.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_is_block_device.bin
