/*
 *  Check unit test suit for skid_file_metadata_read.h's get_owner() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_get_owner.bin
code/dist/check_sfmr_get_owner.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_get_owner.bin
...
#include "skid_file_metadata_read.h"  // get_owner()
