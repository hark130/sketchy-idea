/*
 *  Check unit test suit for skip_file_metadata_read.h's is_directory() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_is_directory.bin
code/dist/check_sfmr_is_directory.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_is_directory.bin
...
#include "skip_file_metadata_read.h"  // is_directory()
