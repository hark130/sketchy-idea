/*
 *  Check unit test suit for skid_file_metadata_read.h's get_file_type() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_get_file_type.bin
code/dist/check_sfmr_get_file_type.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_get_file_type.bin
...
#include "skid_file_metadata_read.h"  // get_file_type()
...
START_TEST(test_n01_block_device)
{
    ...
    result = get_file_type("/dev/loop0", &errnum);
    ck_assert(S_IFBLK == result);  // This input is *NOT* a regular file
...
START_TEST(test_n02_character_device)
{
    mode_t result = 0;        // Return value from function call
    int errnum = CANARY_INT;  // Errno from the function call
    result = get_file_type("/dev/null", &errnum);
    ck_assert(S_IFCHR == result);  // This input is *NOT* a regular file
    ck_assert_int_eq(0, errnum);  // The out param should be zeroized on success
}
END_TEST
...
START_TEST(test_n03_directory)
{
    ...
    ck_assert(S_IFDIR == result);  // This input is *NOT* a regular file
    ck_assert_int_eq(0, errnum);  // The out param should be zeroized on success
...
START_TEST(test_n04_named_pipe)
{
    ...
    // TEST START
    result = get_file_type(input_abs_path, &errnum);
    ck_assert(S_IFIFO == result);  // This input is a regular file
    ck_assert_int_eq(0, errnum);  // The out param should be zeroized on success
...
START_TEST(test_n05_regular_file)
{
    ...
    // TEST START
    result = get_file_type(input_abs_path, &errnum);
    ck_assert(S_IFREG == result);  // This input is a regular file
    ck_assert_int_eq(0, errnum);  // The out param should be zeroized on success
...
START_TEST(test_n07_symbolic_link)
{
    ...
    // TEST START
    result = get_file_type(input_abs_path, &errnum);
    ck_assert(S_IFREG == result);  // This may not be a regular file but stat() follows links
    ck_assert_int_eq(0, errnum);  // The out param should be zeroized on success
...
START_TEST(test_e01_null_filename)
{
    mode_t result = 0;        // Return value from function call
    int errnum = CANARY_INT;  // Errno from the function call
    result = get_file_type(NULL, &errnum);
    ck_assert(0 == result);  // This input is *NOT* a regular file
...
START_TEST(test_e02_empty_filename)
{
    mode_t result = 0;        // Return value from function call
    int errnum = CANARY_INT;  // Errno from the function call
    result = get_file_type("\0 NOT HERE!", &errnum);
    ck_assert(0 == result);  // This input is *NOT* a regular file
    ck_assert_int_eq(EINVAL, errnum);  // The out param should be zeroized on success
}
END_TEST
...
START_TEST(test_e03_null_errnum)
{
    ...
    result = get_file_type("/dev/loop0", NULL);
    ck_assert(0 == result);  // This input is *NOT* a regular file
}
END_TEST
...
START_TEST(test_s01_missing_filename)
{
    mode_t result = 0;        // Return value from function call
    int errnum = CANARY_INT;  // Errno from the function call
    result = get_file_type("/does/not/exist.txt", &errnum);
    ck_assert(0 == result);  // This input is *NOT* a regular file
...
Suite *get_file_type_suite(void)
{
    Suite *suite = NULL;
    TCase *tc_core = NULL;

    suite = suite_create("SFMR_Get_File_Type");
