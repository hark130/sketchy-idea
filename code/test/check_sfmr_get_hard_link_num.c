/*
 *  Check unit test suit for skip_file_metadata_read.h's get_hard_link_num() function.
 *
 *  Copy/paste the following from the repo's top-level directory...

make -C code dist/check_sfmr_get_hard_link_num.bin
code/dist/check_sfmr_get_hard_link_num.bin && CK_FORK=no valgrind --leak-check=full --show-leak-kinds=all code/dist/check_sfmr_get_hard_link_num.bin
...
#include "skip_file_metadata_read.h"  // get_hard_link_num()
...
{
    // LOCAL VARIABLES
    nlink_t result = 0;                    // Return value from function call
    int errnum = CANARY_INT;               // Errno from the function call
    nlink_t exp_result = 0;                // Expected results
    char input_path[] = { "/dev/loop0" };  // Test case input

    // SETUP
    exp_result = get_shell_hard_links(input_path, &errnum);

    // VALIDATION
    // It is important get_shell_hard_links() succeeds
    ck_assert_msg(0 == errnum, "get_shell_hard_links() failed with [%d] %s",
                  errnum, strerror(errnum));
    ck_assert_msg(exp_result >= 0, "get_shell_hard_links() provided an invalid value of %ld",
                  exp_result);
    errnum = CANARY_INT;  // Reset this temp var

    // TEST START
    result = get_hard_link_num(input_path, &errnum);
    ck_assert_msg(exp_result == result, "get_hard_link_num() returned %ld instead of %ld",
                  result, exp_result);
...
START_TEST(test_n03_directory)
{
    ...
    exp_result = get_shell_hard_links(input_abs_path, &errnum);
    // It is important get_shell_hard_links() succeeds
    ck_assert_msg(0 == errnum, "get_shell_hard_links() failed with [%d] %s",
                  errnum, strerror(errnum));
    ck_assert_msg(exp_result >= 0, "get_shell_hard_links() provided an invalid value of %ld",
    ...
    // TEST START
    result = get_hard_link_num(input_abs_path, &errnum);
    ck_assert_msg(exp_result == result, "get_hard_link_num() returned %ld instead of %ld",
                  result, exp_result);
    ck_assert_int_eq(0, errnum);  // The out param should be zeroized on success
...
START_TEST(test_n07_symbolic_link)
{
    ...
    ck_assert_msg(0 == errnum, "resolve_to_repo(%s, %s) failed with [%d] %s", repo_name,
                  input_rel_path, errnum, strerror(errnum));
    exp_result = get_shell_hard_links(actual_abs_path, &errnum);  // Special because of sym link
    // It is important get_shell_hard_links() succeeds
    ck_assert_msg(0 == errnum, "get_shell_hard_links() failed with [%d] %s",
                  errnum, strerror(errnum));
    ck_assert_msg(exp_result >= 0, "get_shell_hard_links() provided an invalid value of %ld",
...
START_TEST(test_e01_null_filename)
{
    ...
    int errnum = CANARY_INT;  // Errno from the function call
    nlink_t exp_result = 0;   // Expected results
    result = get_hard_link_num(NULL, &errnum);
    ck_assert_msg(exp_result == result, "get_hard_link_num() returned %ld instead of %ld",
                  result, exp_result);
...
START_TEST(test_e02_empty_filename)
{
    ...
    int errnum = CANARY_INT;  // Errno from the function call
    nlink_t exp_result = 0;   // Expected results
    result = get_hard_link_num("\0 NOT HERE!", &errnum);
    ck_assert_msg(exp_result == result, "get_hard_link_num() returned %ld instead of %ld",
                  result, exp_result);
    ck_assert_int_eq(EINVAL, errnum);  // The out param should be zeroized on success
}
...
START_TEST(test_e03_null_errnum)
{
    nlink_t result = 0;       // Return value from function call
    nlink_t exp_result = 0;   // Expected results
    result = get_hard_link_num("/dev/loop0", NULL);
    ck_assert_msg(exp_result == result, "get_hard_link_num() returned %ld instead of %ld",
                  result, exp_result);
}
END_TEST
...
START_TEST(test_s01_missing_filename)
{
    nlink_t result = 0;       // Return value from function call
    int errnum = CANARY_INT;  // Errno from the function call
    result = get_hard_link_num("/does/not/exist.txt", &errnum);
    ck_assert(0 == result);
...
Suite *get_hard_link_num_suite(void)
{
    Suite *suite = NULL;
    TCase *tc_core = NULL;

    suite = suite_create("SFMR_Get_Hard_Link_Num");
...
int main(void)
{
    ...
    // Relative path for this test case's input
    char log_rel_path[] = { "./code/test/test_output/check_sfmr_get_hard_link_num.log" };
