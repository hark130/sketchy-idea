...
int make_a_symlink(const char *target_path, const char *link_path)
{
    ...
    char command[] = { "ln -s " };  // The base shell command
    size_t target_link_len = 0;     // Lenght of the concatenated target and link paths
    char *target_link = NULL;       // Concatenated target and link paths
    char output[512] = { 0 };       // Output from the command

    // INPUT VALIDATION
    results = validate_name(target_path);
    if (ENOERR == results)
    {
        results = validate_name(link_path);
    }
    ...
    if (ENOERR == results)
    {
        target_link_len = strlen(target_path) + 1 + strlen(link_path) + 1;  // 1 for ' ', 1 for nul
        target_link = alloc_devops_mem(target_link_len, sizeof(char), &results);
        if (ENOERR != results)
        {
            PRINT_ERROR(The call to alloc_devops_mem() failed);
            PRINT_ERRNO(results);
    ...
    // Form appended command
    if (ENOERR == results)
    {
        snprintf(target_link, target_link_len, "%s %s", target_path, link_path);
    }
    // Execute command
    if (ENOERR == results)
    {
        results = run_command_append(command, target_link, output,
                                     sizeof(output) / sizeof(*output));
        if (results)
        {
            PRINT_ERROR(The call to run_command_append() failed);
            PRINT_ERRNO(results);
            FPRINTF_ERR("%s - The call to %s%s output: %s\n", DEBUG_ERROR_STR, command,
                        target_link, output);
        }
    }
    ...
    if (NULL != target_link)
    {
        free_devops_mem((void **)&target_link);
    }
    return results;
}
